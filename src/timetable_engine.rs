...
use calamine::{DataType, Dimensions, Reader, Xlsx, open_workbook};

use crate::{generate_pdf, input_mod::InputMod, model::Period, utils::sort_table};

pub fn extract_timetable(args: InputMod) {
    println!("Processing timetable... {}", args.file_path.display());
    ...
    for (sheet, value) in sheets {
        ...
        for (row, col, data) in value.used_cells() {
            ...
            let is_match = data
                ...
                .unwrap()
                .to_lowercase()
                .contains(args.level.to_lowercase().as_str());
    ...
    generate_pdf::generate_timetable_pdf(&classes, &args.level.to_uppercase());
}
